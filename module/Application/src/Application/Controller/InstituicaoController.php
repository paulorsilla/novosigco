	...
	public function buscaempregadosAction() {
		...
		if ($request->isPost ()) {
			...
			foreach ( $empregados as $key => $empregado ) {
				$representantes [$key] = $empregado->getMatricula () . "&" . $empregado->getNome () . "&" . $empregado->getCargo ()->cargo . "&" . $empregado->getEmail () . "&" . $empregado->getTelefone () . "&" . $empregado->getTelefoneCelular ();
			}
			$response->setContent ( \Zend\Json\Json::encode ( array (
					'dataType' => 'json',
					'response' => true,
